...
instance (PointVec v d) => ToPointVec v v d where
  toPointVec = id


kMeans :: (Tv.Traversable c, PointVec v d, Ord d, Ord v, Fractional d)
       => c v
       -> [v]
       -> [v]
...
statsForResults :: (PointVec v d, Tv.Traversable c, Ord d, Ord v, Fractional d)
